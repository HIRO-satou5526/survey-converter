...
function bindCsv(){$('runCsvBtn').addEventListener('click',runCsv);$('downloadCsvBtn').addEventListener('click',downloadCsv)}
function bindFavorites(){$('clearFavoritesBtn').addEventListener('click',()=>{if(confirm('保存地点をすべて削除しますか？')){localStorage.removeItem(LS.fav);loadFavorites()}})}
function syncMode(){const mode=getMode();$('xyInputs').classList.toggle('hidden',mode!=='xy2ll');$('llInputs').classList.toggle('hidden',mode!=='ll2xy')}
async function runSingle(){try{setStatus('計算中');const input=readInput();lastResult=await convertPoint(input);localStorage.setItem(LS.last,JSON.stringify(lastResult));renderResult(lastResult);setStatus('完了')}catch(e){showError(e.message);setStatus('要確認')}}
function readInput(){const mode=getMode(),zone=num($('zone').value,'系番号'),heightType=$('heightType').value,heightValue=num($('heightValue').value,'高さ');if(mode==='xy2ll')return{mode,zone,x:num($('x').value,'X座標'),y:num($('y').value,'Y座標'),heightType,heightValue};return{mode,zone,lat:num($('lat').value,'緯度'),lon:num($('lon').value,'経度'),heightType,heightValue}}
...
function renderResult(r){const rows=[['緯度',fmt(r.latitude,8),'°'],['経度',fmt(r.longitude,8),'°'],['X座標',fmt(r.x,3),'m'],['Y座標',fmt(r.y,3),'m'],['楕円体高',fmt(r.ellipsoidHeight,3),'m'],['ジオイド高2011',fmt(r.geoid2011,3),'m'],['ジオイド高2024',fmt(r.geoid2024,3),'m'],['標高（JGD2011）',fmt(r.height2011,3),'m'],['標高（JGD2024）',fmt(r.height2024,3),'m']];$('resultList').className='resultList';$('resultList').replaceChildren(...rows.map(([k,v,u])=>rowEl(k,`${v} ${u}`)),noteEl(`ジオイド取得元: ${r.source}`))}
function rowEl(k,v){const d=document.createElement('div');d.className='row';d.innerHTML=`<div><b>${k}</b><span>${v}</span></div>`;const b=document.createElement('button');b.type='button';b.textContent='コピー';b.onclick=()=>copyText(v);d.append(b);return d}
